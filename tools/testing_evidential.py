...
class RadarPoseTester:
    ...
    def predict_sequence(self, combination: list[str], keypoints: bool = False, return_preds: bool = False):
        ...
        with torch.no_grad():
            for i in tqdm(range(0, radar.size(0) - self.seq_len, batch_size), desc=f"Predicting {'_'.join(combination)}"):
                ...
                out, cdf = self.model(batch)
                preds.append(out[0])
                vars.append(torch.var(cdf, dim = -1))
                cdfs.append(cdf)
    ...
    def run_evaluation(self, parts, angles, actions, reps, model_path: str):
        ...
        np.save("calibration_analysis/mu_testing_evd.npy", np.concatenate(all_preds, axis=0))
        np.save("calibration_analysis/gt_testing_evd.npy", np.concatenate(all_gts, axis=0))
        np.save("calibration_analysis/var_testing_evd.npy", np.concatenate(all_vars, axis=0))
        np.save("calibration_analysis/cdf_testing_evd.npy", np.concatenate(all_cdfs, axis=0))
...
if __name__ == "__main__":
    ...
    res = tester.run_evaluation(
        parts= ["p2", "p1"],#["p1"], #, "p2", "p12"],
        angles=["an0", "an1", "an2"],
        actions=["ac1", "ac2", "ac3", "ac4", "ac5", "ac6", "ac7", "ac8", "ac9"],
        reps=["r0", "r1"],
        model_path=os.path.join(HPECKPT, 
                                "/home/jonas/code/RadProPoser/trainedModels/humanPoseEstimation/evidential/correct")
    )
