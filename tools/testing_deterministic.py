...
def evidential_uncertainty(v: torch.Tensor,
    ...
    # Compute aleatoric component of uncertainty
...
class RadarPoseTester:
    def __init__(self,
        ...
        Args:
            ...
            probabilistic: if True, model is expected to return (..., mu, var);
                           if False, model(batch) should return only mu
        """
    ...
    def predict_sequence(self, combination: list[str], keypoints: bool = False, return_preds: bool = False):
        ...
        self.model.eval()
        with torch.no_grad():
            for i in tqdm(range(0, radar.size(0) - self.seq_len, batch_size),
                          desc=f"Predicting {'_'.join(combination)}"):
                end = min(i + batch_size, radar.size(0) - self.seq_len)
                batch = torch.stack([radar[j:j+self.seq_len] for j in range(i, end)])

                output = self.model(batch)
                if self.probabilistic:
                    # unpack probabilistic outputs
                    _, _, _, mu, var = output
                else:
                    # deterministic model returns only mu
    ...
    def run_evaluation(self, parts, angles, actions, reps, model_path: str):
        ...
        np.save("prediction_data/all_predictions_validation_LSTM.npy", np.concatenate(all_preds, axis=0))
        np.save("prediction_data/all_ground_truths_LSTM.npy", np.concatenate(all_gts, axis=0))
        np.save("prediction_data/all_sigmas_validation_LSTM.npy", np.concatenate(all_vars, axis=0))

        return results_by_participant


if __name__ == "__main__":
    tester = RadarPoseTester(root_path=PATHRAW, probabilistic=False)
    ...
    res = tester.run_evaluation(
        ...
        reps=["r0", "r1"],
        model_path=os.path.join(HPECKPT, "/home/jonas/code/RadProPoser/trainedModels/humanPoseEstimation/CNN_LSTM/correct")
    )
